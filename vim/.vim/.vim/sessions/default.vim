...
nnoremap ,gt :FzfRg
nnoremap ,p :FzfGitFiles --exclude-standard --others --cached
nnoremap ,T :FzfTags
nnoremap ,t :FzfBTags
nnoremap ,h :FzfHistory
nnoremap ,b :FzfBuffers
...
%argdel
$argadd .
edit src/utils/emailActions/sendForwardErrorEmail.ts
...
set winwidth=1
argglobal
balt src/typeProcessors/utils/getUser.ts
setlocal keymap=
setlocal noarabic
setlocal autoindent
setlocal backupcopy=
...
setlocal statusline=%{lightline#link()}%#LightlineLeft_active_0#%(\ %{lightline#mode()}\ %)%{(&paste)?\"|\":\"\"}%(\ %{&paste?\"PASTE\":\"\"}\ %)%#LightlineLeft_active_0_1#%#LightlineLeft_active_1#%(\ %{coc#status()}\ %)%{coc#status()!=#\"\"&&(CocCurrentFunction()!=#\"\"||(&readonly)||1||(&modified||!&modifiable))?\"|\":\"\"}%(\ %{CocCurrentFunction()}\ %)%{CocCurrentFunction()!=#\"\"&&((&readonly)||1||(&modified||!&modifiable))?\"|\":\"\"}%(\ %R\ %)%{(&readonly)&&(1||(&modified||!&modifiable))?\"|\":\"\"}%(\ %t\ %)%{(&modified||!&modifiable)?\"|\":\"\"}%(\ %M\ %)%#LightlineLeft_active_1_2#%#LightlineMiddle_active#%=%#LightlineRight_active_2_3#%#LightlineRight_active_2#%(\ %{&ff}\ %)%{1||1?\"|\":\"\"}%(\ %{&fenc!=#\"\"?&fenc:&enc}\ %)%{1?\"|\":\"\"}%(\ %{&ft!=#\"\"?&ft:\"no\ ft\"}\ %)%#LightlineRight_active_1_2#%#LightlineRight_active_1#%(\ %3p%%\ %)%#LightlineRight_active_0_1#%#LightlineRight_active_0#%(\ %3l:%-2c\ %)
...
normal! 024|
tabnext 1
badd +20 src/handlers/handlerHelpers/communicationTypeHandler.ts
badd +15 src/typeProcessors/processCreateTicketData.ts
badd +9 src/typeProcessors/utils/getUser.ts
badd +11 src/utils/emailActions/sendForwardErrorEmail.ts
